...
const PRODUCTS: Product[] = [
  {
    id: "p1",
    name: "Minimal Linen Tote Bag",
    seller: "aurora_styles",
    sellerAvatar: "aurora",
    price: 89000,
    originalPrice: 129000,
    image: "https://api.dicebear.com/9.x/shapes/svg?seed=bag1&backgroundColor=b6e3f4",
    rating: 4.8,
    reviews: 312,
    category: "Fashion",
    badge: "Best Seller",
  ...
  },
  {
    ...
    image: "https://api.dicebear.com/9.x/shapes/svg?seed=ear2&backgroundColor=c0aede",
    rating: 4.6,
    reviews: 876,
    category: "Tech",
    badge: "30% Off",
  ...
  },
  {
    ...
    sellerAvatar: "mia",
    price: 145000,
    image: "https://api.dicebear.com/9.x/shapes/svg?seed=mug3&backgroundColor=ffd5dc",
    rating: 4.9,
    reviews: 98,
    category: "Home",
    badge: "New",
  ...
  },
  {
    ...
    image: "https://api.dicebear.com/9.x/shapes/svg?seed=serum4&backgroundColor=d1f4cc",
    rating: 4.7,
    reviews: 544,
    category: "Beauty",
    sold: 2100,
  },
  {
    ...
    sellerAvatar: "lamp",
    price: 275000,
    image: "https://api.dicebear.com/9.x/shapes/svg?seed=lamp5&backgroundColor=ffecc8",
    rating: 4.5,
    reviews: 203,
    category: "Home",
    sold: 780,
  },
  {
    ...
    image: "https://api.dicebear.com/9.x/shapes/svg?seed=match6&backgroundColor=b6e3b6",
    rating: 4.8,
    reviews: 421,
    category: "Food",
    badge: "Popular",
    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto max-h-[45vh] px-5 py-3 flex flex-col gap-3">
          {cart.length === 0 ? (
            <div className="text-center py-10 text-muted-foreground text-sm">Keranjang kamu kosong</div>
          ) : (
            cart.map((item) => (
              <div key={item.id} className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button type="button" onClick={() => onQtyChange(item.id, 1)} className="w-7 h-7 rounded-lg bg-muted flex items-center justify-center font-bold hover:bg-primary/20 transition-smooth">+</button>
                  <button type="button" onClick={() => onRemove(item.id)} className="w-7 h-7 rounded-lg bg-destructive/10 flex items-center justify-center text-destructive hover:bg-destructive/20 transition-smooth ml-1">
                    <X size={13} />
                  </button>
                </div>
              </div>
        ...
        {cart.length > 0 && (
          <div className="px-5 pb-8 pt-3 border-t border-border space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-base font-display font-bold text-foreground">{formatRupiah(total)}</span>
            </div>
            ...
            {checked ? (
              <motion.div
                ...
                className="w-full py-3.5 rounded-2xl bg-green-500/20 border border-green-500/30 text-green-400 font-semibold text-sm text-center"
              >
              ...
              <button
                ...
                className="w-full py-3.5 rounded-2xl bg-primary text-primary-foreground font-display font-bold text-sm hover:opacity-90 transition-smooth"
              >
                Checkout Sekarang →
              </button>
            )}
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-muted">
        ...
        {product.badge && (
          <span className="absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full bg-primary text-primary-foreground">{product.badge}</span>
        )}
        {discount > 0 && (
          <span className="absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground">-{discount}%</span>
        )}
        <button
          type="button"
          onClick={() => setWishlisted((v) => !v)}
          className="absolute bottom-2 right-2 w-8 h-8 rounded-full bg-card/80 backdrop-blur-sm flex items-center justify-center transition-smooth"
        >
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        <p className="text-xs text-muted-foreground flex items-center gap-1">
          <img src={`https://api.dicebear.com/9.x/notionists/svg?seed=${product.sellerAvatar}`} className="w-4 h-4 rounded-full" alt="" />
          {product.seller}
        </p>
        ...
        <div className="mt-auto pt-1">
          <div className="flex items-baseline gap-1.5">
            <span className="text-sm font-display font-bold text-primary">{formatRupiah(product.price)}</span>
            {product.originalPrice && (
              <span className="text-[11px] text-muted-foreground line-through">{formatRupiah(product.originalPrice)}</span>
            )}
          </div>
        ...
        <button
          ...
          className="w-full mt-1 py-2 rounded-xl bg-primary/10 border border-primary/20 text-primary text-xs font-semibold hover:bg-primary hover:text-primary-foreground transition-smooth"
        >
          + Keranjang
        </button>
      </div>
    ...
    <Layout>
      ...
      <div className="flex gap-2 px-4 mt-4 overflow-x-auto no-scrollbar pb-1">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={cn(
              "px-4 py-1.5 rounded-full text-xs font-semibold shrink-0 transition-smooth border",
              activeCategory === cat
      ...
      <div className="flex gap-3 px-4 mt-4">
        {[
          ...
          { icon: Tag, label: "Brand", value: "50+" },
          { icon: Truck, label: "Gratis Ongkir", value: "Ya!" },
        ].map((s) => (
      ...
      <AnimatePresence>
        {cartOpen && (
          <CartDrawer cart={cart} onClose={() => setCartOpen(false)} onQtyChange={changeQty} onRemove={removeItem} />
        )}
      </AnimatePresence>
    </Layout>
  );
