...
import {
  AtSign, Camera, CheckCircle, Grid3x3, Heart,
  MessageCircle, Settings, Share2, ShoppingBag,
...
export default function ProfilePage() {
  ...
  function requestSell() {
    setSellOpen(false);
    toast("Permintaan jualan dikirim ke JoyDev! Tunggu persetujuan ya 🙏");
  }
    ...
    <Layout>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <div className="bg-secondary/10 border-b border-secondary/20 px-4 py-2 flex items-center gap-2">
          <span className="text-[10px] font-bold text-secondary uppercase tracking-wide">🚧 Dalam Tahap Pengembangan</span>
          <span className="text-[10px] text-muted-foreground">by JoyDev</span>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-5 mt-3">
                {[{ label: "Postingan", value: 12 }, { label: "Pengikut", value: "1.2K" }, { label: "Mengikuti", value: 248 }].map(({ label, value }) => (
          ...
          <div className="mt-4 flex gap-2">
            {isSelf ? (
              <>
                ...
                <button type="button" onClick={() => setSellOpen(true)} className="flex-1 h-10 rounded-xl bg-secondary/10 border border-secondary/30 text-secondary text-sm font-semibold flex items-center justify-center gap-1.5 hover:bg-secondary/20 transition-smooth">
              ...
              <>
                <button
                  type="button"
                  onClick={() => { setFollowing(!following); toast(following ? "Berhenti mengikuti" : "Mulai mengikuti!"); }}
        ...
        <div className="flex bg-card border-b border-border">
          {([["posts", Grid3x3, "Postingan"], ["products", ShoppingBag, "Produk"]] as const).map(([t, Icon, label]) => (
            <button key={t} type="button" onClick={() => setTab(t)}
              className={cn("flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-smooth border-b-2", tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground")}
            >
              <Icon size={16} /> {label}
            </button>
          ))}
        ...
        {tab === "products" && (
          <div className="flex flex-col items-center justify-center py-16 gap-4 px-6">
            ...
            {isSelf && (
              <button type="button" onClick={() => setSellOpen(true)} className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold">
                Daftar Penjual
              </button>
            )}
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" onClick={() => setEditOpen(false)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", bounce: 0.1 }} className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-card rounded-t-3xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto pb-10">
                ...
                <div>
                  <label className="text-xs text-muted-foreground font-medium">Bio <span className="text-primary text-[10px]">(ketik @ untuk tag orang)</span></label>
                  <div className="mt-1">
                    <TagTextarea value={editBio} onChange={setEditBio} placeholder="Tulis bio kamu..." />
                  </div>
                </div>
        ...
        {sellOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" onClick={() => setSellOpen(false)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", bounce: 0.1 }} className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-card rounded-t-3xl p-5 pb-10" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-xs text-muted-foreground text-center mb-4">Dengan mendaftar, kamu setuju dengan syarat & ketentuan penjual Social Mart</p>
              <button type="button" onClick={requestSell} className="w-full py-3.5 rounded-2xl bg-secondary text-secondary-foreground font-bold text-sm">
