...
import { Camera, ChevronRight, Image, Plus, Search, Tag, Users, X } from "lucide-react";
...
export default function StoryPage() {
  ...
  const [viewingStory, setViewingStory] = useState<typeof SAMPLE_STORIES[0] | null>(null);
      ...
      <div className="fixed inset-0 z-50 bg-black flex flex-col">
        ...
        <div className="absolute top-8 left-4 right-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Avatar
              src={`https://api.dicebear.com/9.x/notionists/svg?seed=${viewingStory.seed}`}
          ...
          <button type="button" onClick={() => setViewingStory(null)} className="p-2">
      ...
      <Layout>
        ...
        {step === "create" && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-6">
            ...
            <p className="text-muted-foreground text-sm text-center">Pilih foto atau video untuk story kamu</p>
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => fileRef.current?.click()}
                className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-primary text-primary-foreground font-semibold text-sm"
              >
              ...
              <button
                type="button"
                onClick={() => fileRef.current?.click()}
                className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-muted border border-border text-foreground font-semibold text-sm"
              >
        ...
        {step === "preview" && preview && (
          <div className="flex flex-col gap-4 p-4">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-muted border border-border text-sm font-semibold text-foreground"
            >
              <Tag size={16} className="text-primary" />
              Tag Orang
              {taggedUsers.length > 0 && (
                <span className="ml-auto text-xs text-primary">{taggedUsers.length} ditag</span>
              )}
            </button>
    ...
    <Layout>
      ...
      <div className="px-4 py-4">
        ...
        <button
          ...
          className="flex items-center gap-3 w-full p-3 rounded-2xl bg-muted/50 border border-border hover:border-primary/40 transition-smooth"
        >
          <div className="relative">
        ...
        <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3 mt-5">Story Teman</h2>
        <div className="space-y-2">
          {SAMPLE_STORIES.map((story, i) => (
            <motion.button
              ...
              className="flex items-center gap-3 w-full p-3 rounded-2xl hover:bg-muted/50 transition-smooth"
            >
              ...
              <div className="text-left flex-1">
                <p className={cn("text-sm font-semibold", story.viewed ? "text-muted-foreground" : "text-foreground")}>
                  {story.username}
                </p>
